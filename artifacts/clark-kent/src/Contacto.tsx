...
import { useGetContactSettings, useListSocialLinks } from "@workspace/api-client-react";
import { buildInfoHrefForNumber } from "./utils/whatsapp";
...
export default function Contacto() {
  ...
  const socialLinksQuery = useListSocialLinks();
  const [slide, setSlide] = useState(0);
  const [isVisible, setIsVisible] = useState(false);
  const sectionRef = useRef<HTMLElement>(null);
  const settings = contactSettingsQuery.data;
  const whatsappNumber = settings?.whatsappNumber || WA_NUMBER;
  const phoneDisplay = settings?.phone || PHONE_DISPLAY;
  const contactHours = settings
    ? lang === "es"
      ? [settings.businessHoursWeekdaysEs, settings.businessHoursWeekendEs]
      : [settings.businessHoursWeekdaysEn, settings.businessHoursWeekendEn]
    : t.hours;
  const contactInfo = t.info.map((item) => {
    if (!settings) return item;
    if (item.icon === "location") {
      return { ...item, value: lang === "es" ? settings.locationEs : settings.locationEn };
    }
    if (item.icon === "email") {
      return { ...item, value: settings.email, href: `mailto:${settings.email}` };
    }
    if (item.icon === "phone") {
      return { ...item, value: phoneDisplay, href: `tel:${phoneDisplay.replace(/\s+/g, "")}` };
    }
    return { ...item, value: phoneDisplay, href: `https://wa.me/${whatsappNumber}` };
  });
  const contactSocialLinks = socialLinksQuery.data?.find((group) => group.category === "contact")?.links ?? [];
  const socialHref = (icon: string, fallback: string) =>
    icon === "instagram"
      ? "https://www.instagram.com/fishertstudio?stkn=bW9uM2trdGVhbnFt"
      : contactSocialLinks.find((link) => link.icon === icon)?.url || fallback;
                ...
                <div className="cslider-track" style={{ transform: `translateX(${slide * -50}%)` }}>
                  ...
                  <div className="cslider-slide">
                    ...
                    <div className="contacto-social-row">
                      <a href={socialHref("instagram", "https://www.instagram.com/fishertstudio?stkn=bW9uM2trdGVhbnFt")} target="_blank" rel="noopener noreferrer"
                      ...
                      <a href={socialHref("github", "https://github.com/fishertstudio")} target="_blank" rel="noopener noreferrer"
                      ...
                      <a href={buildInfoHrefForNumber(whatsappNumber, lang)} target="_blank" rel="noopener noreferrer"
                  ...
                  <div className="cslider-slide cslider-slide--booking">
                    ...
                    <a
                      href={`https://wa.me/${whatsappNumber}?text=${buildBookingMsg(lang)}`}
                      target="_blank"
